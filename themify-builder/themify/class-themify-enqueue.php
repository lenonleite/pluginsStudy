...
class Themify_Enqueue_Assets {
    ...
    public static $SWIPER_VERSION='5.3.7';
    ...
    public static function wp_footer() {
		...
		if (self::$concateFile !== null) {
			...
			if ($key !== '') {
				...
				$regenerate=false;
				if($exist===true){
					$regenerate=!apply_filters('themify_concate_css',true,self::$concateFile);//opposite logic for backward compatibility
	...
	*/
	...
	public static function cron(){
		...
		$dir=$upload_dir.self::$concateDir;
		unset($_arr);
		clearstatcache();
		if (is_dir($dir) && ($handle = opendir($dir))) {
			...
			$globalKey='-'.crc32($globalKey).'-';
			while (false !== ($f = readdir($handle))) {
				if ($f !== '.' && $f !== '..' && strpos($f,$globalKey,5)===false) { 
					unlink($dir.$f);
				}
			}
			closedir($handle);
			unset($object,$dir,$globalKey);
		}
