...
(function (Themify,document) {
    'use strict';
    const _click=!Themify.isTouch?'click':(window.PointerEvent?'pointerdown':'touchstart'),
		IS_IOS=/iPhone|iPad|iPod|Mac OS/i.test(window.navigator.userAgent),
		COOKIE='tf_video_',
		...
		loadMetaData=function(el,opt){
			const fr=document.createDocumentFragment(),
				...
				hasVolume=IS_IOS===false || Themify.device!=='mobile';
				...
				if(!Themify.isTouch){
					...
					if(ver<13.5){
						...
						progressWrap.addEventListener(_click,__changeCallback,{passive:true});
						if(hasVolume===true){
							volumeInner.addEventListener(_click,__changeCallback,{passive:true});
						}
					}
				}
				...
				if (!el.hasAttribute('disablePictureInPicture') && document.pictureInPictureEnabled) {
					...
					pip.addEventListener(_click,pipCallback,{passive:true});
				...
				if (window.WebKitPlaybackTargetAvailabilityEvent) {
					el.addEventListener('webkitplaybacktargetavailabilitychanged', function(e) {
						if(e.availability==='available'){
							...
							airPlay.addEventListener(_click, function() {
