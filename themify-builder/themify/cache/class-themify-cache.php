...
class TFCache {
    ...
    public static function hooks() {
		...
		if(isset($_GET['tf-cache']) && $_GET['tf-cache']==='2' && is_user_logged_in() && current_user_can('manage_options')){
	...
	public static function cache_menu($wp_admin_bar){
	    ...
	    if(!$is_admin){
            ...
            if(!is_date()){
                ...
                if(empty($post_id) || (!empty($post_id) && !current_user_can('edit_'.$type, $post_id))){
                    $is_admin=true;
                }
            }
        }
