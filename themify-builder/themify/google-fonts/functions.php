...
function themify_get_google_fonts_file() {
     ...
     * @param string $fonts
     */
    return apply_filters( 'themify_google_fonts_file',dirname( __FILE__ ).'/'. $fonts.'.php' );
}
...
function themify_grab_remote_google_fonts() {
	return include( themify_get_google_fonts_file() );
}
...
function themify_get_google_web_fonts_list() {
	...
	foreach ( $fonts as $k=>$f ) {
	    $google_fonts_list[] = array(
		    'value' => $k,
		    'name' => $k,
		    'variant' => isset($f['variants'])?$f['variants']:array()
	    );
	}
