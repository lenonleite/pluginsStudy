    ...
    ThemifyConstructor = {
        ...
        set(value) {
            try {
                ...
                localStorage.setItem(this.key, JSON.stringify( {val: value, ver: themify_vars.version, h: Themify.hash(m)}));
                return true;
            }
            catch (e) {
                return false;
        ...
        run(type, save_opt) {
            ...
            Themify.body.one('themify_builder_lightbox_close', function () {
                ...
                rememberedEl = data=self.values = self.type = self.component = self.is_new = null;
                self.tabs.click = 0;
        ...
        gradient: {
            controlChange(self, gradient, input, clear, type, angle, circle, text, update) {
                ...
                const instance = $gradient.data('themifyGradient'),
					callback = function (val) {
						...
						if (!p.classList.contains('tb_gradient_angle_container')) {
							p = angle;
						}
						if (val === 'radial') {
                ...
                if (update) {
                    ...
                    angle.addEventListener(ev['mousedown'], function(e) {
							e.stopImmediatePropagation();
							let _circle=this.parentNode.querySelector('.tb_gradient_circle');
							if(!_circle){
								let v=this.value;
								...
								tmp1.className='tb_gradient_dot';
								_circle.className='tb_gradient_circle';
								tmp2.className='tb_gradient_circle_wrapper';
							...
							_circle.addEventListener(e.type,function(e){
								if(e.type==='touchstart' || e.which===1){
									let box=this.getBoundingClientRect(),
										...
										timer,
										_wrapper=this.parentNode.querySelector('.tb_gradient_circle_wrapper'),
										_dot=this.parentNode.querySelector('.tb_gradient_dot');
            ...
            render(data, self) {
                const wrap = document.createElement('div'),
					...
					angleWrapper=document.createElement('div'),
				...
				angleWrapper.tabIndex=-1;
				angleWrapper.className='tb_gradient_angle_container tf_rel';
                angle.type = 'number';
        ...
        font_select: {
            ...
            loadGoogleFonts( fontFamilies ) {
				...
				if(result.google.length > 0){
					...
					Themify.LoadCss(window.location.protocol+'//fonts.googleapis.com/css?family='+encodeURI(result.google.join('|'))+'&display=swap',false,null,null,loading);
				}
				if(result.cf.length > 0){
					Themify.LoadCss(themifyBuilder.cf_api_url+encodeURI(result.cf.join('|')),false,null,null,loading);
				}
            },
            controlChange(select, preview, pw, self) {
				...
				$combo[0].addEventListener('click', function (e) {
					...
					if(target.classList.contains('themify-combo-item')){
						...
						if ('cf' === type && value && _this.loaded_fonts.indexOf(value) === -1) {
							_this.loadGoogleFonts(value);
						}
						_this.updateFontVariant(value, select.closest('.tb_tab').getElementsByClassName('font-weight-select')[0], self,type);
						setTimeout(function () {
				...
				$combo[0].addEventListener('mouseover', function (e) {
                    ...
                    if(target.classList.contains('themify-combo-item')){
                        ...
                        if (value) {
                            ...
                            if (!target.classList.contains('tb_font_loaded')) {
                                ...
                                if (_this.fonts.indexOf(value) === -1) {
                                    ...
                                    if (type && type!== 'webfont') {
										...
										if(url!==''){
											topWindow.Themify.LoadCss(url,false,null,null,callback);
										}
                                    }
                                    else {
                                        callback(value);
            ...
            apply_all(border, item) {
                const items = item.getElementsByTagName('input'),
                        disable = function (is_all, event) {
                            for (let i = items.length - 1; i >-1; --i) {
								items[i].parentNode.classList.toggle('_tb_disable',is_all);
                            }
                            if (is_all === true) {
