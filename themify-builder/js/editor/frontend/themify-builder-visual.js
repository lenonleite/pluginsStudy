(function ($, Themify, window, topWindow, document, api, Common) {
    ...
    api.Mixins.Frontend = {
        ...
        restoreHtml(rememberedEl) {
            ...
            const $currentEl = api.liveStylingInstance.$liveStyledElmt,
                batch = Themify.convert(rememberedEl[0].querySelectorAll('[data-cid]'));
    ...
    Themify.body.one('builderiframeloaded.themify', function (e, iframe) {
        ...
        // Disable Links and Submit buttons in live builder
        iframe.contentWindow.document.getElementsByTagName("body")[0].addEventListener('click',function(e){
            const target = e.target,
                el = 'A'===target.tagName?target:target.closest('a');
            if(el && '#' !== el.href && '' !== el.href && el.href.replace(new URL(el.href).hash,'') !== window.top.location.href.replace(location.hash,"") || (target.tagName==='BUTTON' && target.type === 'submit')){
                e.preventDefault();
            }
        })
    });
                        ...
                        getCssValue = function (el) {
                            ...
                            if (is_border === true) {
								const p = el.closest('li'),
									...
									color_val = api.Utils.getColor(p.getElementsByClassName('minicolors-input')[0]);
                                if (style === 'none') {
                                    v = style;
                                }
