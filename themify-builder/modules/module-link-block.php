...
class TB_Link_Block_Module extends Themify_Builder_Component_Module {
    ...
    protected function _visual_template() {
        ?>
            <# data.color = undefined == data.color || data.color=='default' ? 'tb_default_color' : data.color; #>
        <div class="module module-<?php echo $this->slug; ?> {{ data.shape!=='normal'?data.shape:'' }} {{ data.style }} tf_textc">
            <{{ data.link ? 'a' : 'span'}} class="tb_link_block_container ui {{ data.color }}">
            <# if ( data.icon ) { #>
            <i class="tb_link_block_icon"><# print(tb_app.Utils.getIcon(data.icon).outerHTML)#></i>
            <# }
            if ( undefined != data.heading && '' != data.heading ) { #>
            <span class="tb_link_block_heading tf_block">{{{ data.heading }}}</span>
            <# }
            if ( undefined != data.blurb && '' != data.blurb ) { #>
            <span class="tb_link_block_blurb tf_block">{{{ data.blurb }}}</span>
            <# } #>
            </{{ data.link ? 'a' : 'span'}}>
        </div>
        <?php
