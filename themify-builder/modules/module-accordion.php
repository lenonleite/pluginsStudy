...
class TB_Accordion_Module extends Themify_Builder_Component_Module {
	...
	#>
		...
		<li<# if ( isOpen ) { #> class="builder-accordion-active"<# } #>>
			...
			<a href="#acc-{{ tabId }}-{{ i }}" aria-controls="acc-{{ data.cid }}-{{ i }}" aria-expanded="{{ isOpen }}">
		    ...
		    <div data-id="acc-{{ tabId }}-{{ i }}" aria-hidden="{{!isOpen}}" class="accordion-content<# if ( !isOpen ) { #> tf_hide<# } #>">
