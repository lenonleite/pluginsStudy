...
$niteoCS_page_filter 			= get_option('niteoCS_page_filter', '0'); //ok not work
$niteoCS_page_whitelist			= json_decode(get_option('niteoCS_page_whitelist', '[]'), true);//ok not work
$niteoCS_page_whitelist_custom	= json_decode(get_option('niteoCS_page_whitelist_custom', '[]'), true);//ok not work
$niteoCS_page_blacklist			= json_decode(get_option('niteoCS_page_blacklist', '[]'), true);//ok not work
$niteoCS_page_blacklist_custom	= json_decode(get_option('niteoCS_page_blacklist_custom', '[]'), true);//ok not work
$custom_login_url				= get_option('niteoCS_custom_login_url', get_option( 'whl_page' )); //ok BOOOOMMMM \"onmouseover=javascript:alert(1);%20test="
$niteoCS_roles					= json_decode(get_option('niteoCS_roles', '[]'), true);//ok not work
$niteoCS_roles_topbar			= json_decode(get_option('niteoCS_roles_topbar', '[]'), true);//ok not work
$head_scripts					= json_decode(get_option('niteoCS_head_scripts', '[]'), true);//ok not work
$footer_scripts					= json_decode(get_option('niteoCS_footer_scripts', '[]'), true);//ok not work
$bypass							= get_option('niteoCS_bypass', '0');//ok not work
$bypass_id 						= get_option('niteoCS_bypass_id', md5( get_home_url() ));  //ok BOOOOMMMM </code><script>alert(1)</script>
$bypass_expire 					= get_option('niteoCS_bypass_expire', '172800');//ok not work
$topbar_icon 					= get_option('niteoCS_topbar_icon', '1');//ok not work
$topbar_version 				= get_option('niteoCS_topbar_version', 'cmp-topbar-full');//ok not work
$wpautop 						= get_option('niteoCS_wpautop', '1');//ok not work

$cmp_countdown_notif 			= get_option('niteoCS_countdown_notification', '1');//ok not work
$cmp_mode_change_notif 			= get_option('niteoCS_mode_change_notification', '0');//ok not work
$cmp_subscribe_notif 			= get_option('niteoCS_subscribe_notification', '0');//ok not work
$countdown_email 				= get_option('niteoCS_countdown_email_address', get_option( 'admin_email' ));//ok not work
$mode_change_email 				= get_option('niteoCS_mode_change_email_address', get_option( 'admin_email' ));//ok not work
$subscribe_email 				= get_option('niteoCS_subscribe_email_address', get_option( 'admin_email' ));//ok not work
$cmp_cookie_notice_comp			= get_option('cmp_cookie_notice_comp', '1');//ok not work
$cmp_rss 						= get_option('niteoCS_rss_status', '1');//ok not work
$cmp_rest_api 					= get_option('niteoCS_rest_api_status', '1');//ok not work
?>
...
<!--									<input type="text" name="niteoCS_custom_login_url" value="--><?php //echo  $custom_login_url ; ?><!--" class="regular-text"><br>-->
