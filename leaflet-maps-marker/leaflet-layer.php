...
if (!empty($action)) {
  ...
  if ( ($lat_check != NULL) && ($lon_check != NULL) ) {
      ...
      exit($lat_check);
		...
		$wpdb->query( $result );
		$wpdb->query( "OPTIMIZE TABLE `$table_name_layers`" );
		echo EAFLET_WP_ADMIN_URL . 'admin.php?page=leafletmapsmarker_layer&id=' . $oid . '&status=updated&Layername='.urlencode($_POST['name']);exit;
	...
	<form id="layer-add-edit" method="post">
		...
		<table class="widefat">
			...
			<tr>
				...
				<strong><label for="listmarkers"><?php _e('List of markers position','lmm') ?></label></strong><br/>
				<input style="margin-top:1px;" id="listmarkers_hidden" type="radio" name="listmarkers" value="0" <?php checked($llistmarkers, 0); ?>><label for="listmarkers_hidden"><?php _e('hidden','lmm') ?></label><br/>
				<input style="margin-top:1px;" id="listmarkers_below" type="radio" name="listmarkers" value="1" <?php checked($llistmarkers, 1); ?>><label for="listmarkers_below"><?php _e('below the map','lmm') ?></label><br/>
				<input style="margin-top:1px;" id="listmarkers_right" type="radio" name="listmarkers" value="2" disabled="disabled"><label for="listmarkers_right"><?php _e('right to the map','lmm') . _e(' <small>(<a href="' . LEAFLET_WP_ADMIN_URL . 'admin.php?page=leafletmapsmarker_pro_upgrade">' . __('pro version only','lmm') . '</a>)</small>'); ?></label><br/>
				<input style="margin-top:1px;" id="listmarkers_left" type="radio" name="listmarkers" value="3" disabled="disabled"><label for="listmarkers_left"><?php _e('left to the map','lmm') . _e(' <small>(<a href="' . LEAFLET_WP_ADMIN_URL . 'admin.php?page=leafletmapsmarker_pro_upgrade">' . __('pro version only','lmm') . '</a>)</small>'); ?></label><br/>
				<?php
						echo '<small>';
